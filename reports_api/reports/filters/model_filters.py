from reports_api.reports.models import SummitEvent, Speaker, Presentation, RsvpTemplate, Rsvp, \
    EventFeedback, EventCategory, Tag, Metric
import django_filters
from django.db.models import Avg, Count, Exists, FloatField, IntegerField, OuterRef, Q, Subquery
...
class SubQueryAvg(Subquery):
    ...
    def __init__(self, *args, **kwargs):
        Subquery.__init__(self, *args, **kwargs)
        field = kwargs['field']
        self.queryset = self.queryset.annotate(avg=Avg(field)).values("avg")
        self.queryset.query.set_group_by()  # values() adds a GROUP BY we don't want here
...
class PresentationFilter(django_filters.FilterSet):
    ...
    def feedback_filter(self, queryset, name, value):
        return queryset.annotate(feedback_count=Count('feedback'), rate=Avg('feedback__rate')).filter(
            feedback_count__gt=0)

    def video_filter(self, queryset, name, value):
        return queryset.annotate(video_count=Count('materials__presentationvideo')).filter(video_count__gt=0)
...
class SpeakerFilter(django_filters.FilterSet):
    ...
    def has_published_events_from_summit_filter(self, queryset, name, value):
        ...
        speaker_qs = Speaker.objects.filter(presentations__summit__id=value, presentations__published=True).distinct()
        # Moderators point of view
        moderators_qs = Speaker.objects.filter(events__summit__id=value, events__published=True).distinct()
        return speaker_qs.union(moderators_qs)
...
class EventCategoryFilter(django_filters.FilterSet):
    ...
    def feedback_filter(self, queryset, name, value):
        feedbacks = EventFeedback.objects.filter(event__summit__id=OuterRef('summit__id'),
                                                 event__category__id=OuterRef('id'))
...
class TagFilter(django_filters.FilterSet):
    ...
    def has_events_from_summit_filter(self, queryset, name, value):
        queryTmp = queryset.annotate(count=Count('events', filter=Q(events__summit__id=value)))
...
class MetricFilter(django_filters.FilterSet):
    ...
    room_id = django_filters.NumberFilter(field_name='eventmetric__event__location__id')
